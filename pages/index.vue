    ...
    <el-row :gutter="20" class="mt-20" v-for="categoryItem in movieList.data">
          ...
          <div class="panel_hd__right items-center">
            <ul class="items-center">
              <li
                ...
                v-for="item in categoryItem.genres"
              >
                <nuxt-link
                  :to="`/column/${categoryItem.value}/show?t=${item.name}`"
                  >{{ item.name }}</nuxt-link
                >
              </li>
              <li>
                <nuxt-link
                  :to="`/column/${categoryItem.value}`"
                  class="items-center"
                >
          ...
          <el-row :gutter="20">
            <el-col :sm="4" :xs="8" v-for="item in categoryItem.rows">
              <div class="video-list__block">
                <nuxt-link
                  :to="`/column/${item.columnValue}/${item.id}`"
                  class="img-box"
                >
                  <el-image
                    lazy
                ...
                <div class="video-list__detail">
                  ...
                  <p class="text-overflow">
                    <span
                      ...
                      v-for="user in item.casts"
                      >{{ user.actor.name }}</span
                    >
                  </p>
                </div>
              </div>
      ...
      <el-col :span="6" class="hidden-sm-and-down">
        <div class="panel_hd items-center">
          <h3 class="title items-center">{{ categoryItem.name }}榜单</h3>
        </div>
        <ul class="col-pd">
          <li v-for="(item, index) in categoryItem.ranks">
            ...
            >
              ...
              <span class="text-muted" v-else
                >更新至{{ item.episodeCount || 0 }}集</span
              >
            </nuxt-link>
          </li>
        </ul>
...
const [{ data: bannerList }, { data: movieList }] = await Promise.all([
  // banner数据
